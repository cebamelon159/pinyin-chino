...
var Modes = {
  ...
  /* ---- 3. Dictado: se escucha y se escribe el pinyin ---- */
  dictation: {
    id: "dictation", icon: "🎧", name: "Dictado",
    desc: "Escuchas el audio y escribes el pinyin que oíste",
    fits: function () { return Speech.available(); },
    render: function (ctx) {
      var c = ctx.card, body = ctx.body;
      var card = el("div", "card");

      card.appendChild(el("div", "prompt-label", "Escucha y escribe el pinyin"));

      var big = el("button", "icon-btn", "🔊");
      big.style.fontSize = "44px";
      big.style.width = big.style.height = "88px";
      big.onclick = function () { Speech.speak(c.hanzi); };
      card.appendChild(big);
      card.appendChild(el("div", "tap-hint", "Toca para repetir"));
      ...
      input.placeholder = "ni hao / nǐ hǎo";
      card.appendChild(input);

      card.appendChild(el("p", "hint",
        "Los tonos son opcionales: si los escribes se corrigen, si no, se evalúa solo la sílaba."));
      ...
      function check() {
        ...
        var res = checkPinyin(val, plainPinyin(c));
        input.classList.add(res.ok ? "ok" : "bad");
        input.blur();
        ctx.feedback(res.ok, buildDictationNote(c, res));
        ctx.autoGrade(res.ok);
      }
  ...
  /* ---- 4. Traducción ES -> ZH armando la oración ---- */
  build: {
    id: "build", icon: "🧩", name: "Traducir y ordenar",
    desc: "Del español al chino, armando la oración con las palabras dadas",
    ...
    render: function (ctx) {
      ...
      var prompt = el("div", "prompt-block");
      prompt.appendChild(el("div", "prompt-label", "Traduce al chino"));
      prompt.appendChild(el("div", "prompt-es", c.es));
      if (c.pattern) {
  ...
  /* ---- 7. Producción libre: escribes la oración con tu teclado chino ----
     Es el ejercicio que de verdad enseña a construir. Arrastrar fichas ya te
     da las palabras y casi el orden; aquí sales de cero, con pistas si las
     pides, y si fallas te ofrece otra frase con la misma estructura. */
  compose: {
    id: "compose", icon: "⌨️", name: "Escribir la oración",
    desc: "Te doy la frase en español y la escribes en chino con tu teclado",
    fits: function (c) {
      return c.type === "sentence" && !!c.es && countWords(c) >= 2
             && pt_hanziCount(c.hanzi) <= 24;
    },
    render: function (ctx) {
      var c = ctx.card, body = ctx.body;
      ...
      function pista() {
        ...
        if (nivel === 1) {
          ...
          var linea = el("div", "hint-answer hanzi", c.hanzi);
      ...
      function comprobar() {
        ...
        // no coincide: se muestra qué sobra y qué falta, palabra a palabra
      ...
      function terminar(ok, difs) {
        ...
        if (nivel > 0 && ok) {
          extra.appendChild(el("div", "hint-used",
            ...
            (nivel > 1 ? ". Vuelve a esta frase en unos días." : "")));
        }
        // Escribirla bien es escribirla bien: el acierto se reconoce aunque
        // haya pistas. Lo que sí cambia es cuándo vuelve a aparecer.
        ctx.feedback(ok, extra);
        if (!revelado) Speech.speak(c.hanzi);

        // el bucle de insistir: otra frase con la misma estructura
        var acciones = [];
        ...
        acciones.push({
          ...
          // Con más de una pista cuenta como no sabida: así el repaso
          // espaciado la trae de vuelta pronto en vez de darla por aprendida.
      ...
      input.addEventListener("keydown", function (e) {
        // Enter comprueba, salvo que se esté componiendo con el IME
  ...
  /* ---- 6. Escritura: trazar el carácter de memoria ---- */
  writing: {
    id: "writing", icon: "✍️", name: "Escritura",
    desc: "Ves el significado y trazas el carácter con el dedo",
    fits: function (c) {
      // sólo palabras cortas: trazar una oración entera es inviable en pantalla
      if (c.type !== "word" || !c.es) return false;
      var n = 0;
      for (var i = 0; i < c.hanzi.length; i++) if (Strokes.isHanzi(c.hanzi[i])) n++;
      return n >= 1 && n <= 3 && Strokes.available();
    },
    render: function (ctx) {
      var c = ctx.card, body = ctx.body;
      var card = el("div", "card");
      ...
      var prompt = el("div", "prompt-block");
      prompt.appendChild(el("div", "prompt-label", "Escribe el carácter"));
      prompt.appendChild(el("div", "prompt-es", c.es));
      var py = el("div", "card-py");
      py.appendChild(renderPinyin(c));
      prompt.appendChild(py);
      card.appendChild(prompt);

      var chars = [];
      for (var i = 0; i < c.hanzi.length; i++)
        if (Strokes.isHanzi(c.hanzi[i])) chars.push(c.hanzi[i]);

      var grid = el("div", "stroke-grid");
      var writers = [], mistakes = 0, finished = 0;
      ...
      chars.forEach(function (ch, idx) {
        ...
        var w = HanziWriter.create(target, ch, {
          width: 120, height: 120, padding: 5,
          showOutline: false,         // de memoria: sin plantilla
          showCharacter: false,
          strokeColor: "#1b7a43",
          drawingColor: "#c62828",
          charDataLoader: function (x, done) { Strokes.loadChar(x, done); },
        });
        writers.push({ w: w, cell: cell, ch: ch });

        w.quiz({
          onMistake: function () { mistakes++; cell.classList.add("stroke-bad"); },
          onCorrectStroke: function () { cell.classList.remove("stroke-bad"); },
          onComplete: function () {
            cell.classList.add("stroke-done");
            finished++;
            if (finished === chars.length) done();
          },
        });
        cell.appendChild(el("div", "stroke-char hanzi hidden-answer", ch));
      });

      card.appendChild(grid);
      card.appendChild(el("div", "tap-hint", "Traza con el dedo. Si dudas, usa «Ver cómo se escribe»."));
      body.appendChild(card);
      ...
      function done() {
        if (settled) return;
        settled = true;
        var ok = mistakes <= chars.length;   // un fallo por carácter aún cuenta
        ctx.feedback(ok, buildAnswerNote(c));
        Speech.speak(c.hanzi);
        ctx.autoGrade(ok);
      }
      ...
      ctx.setActions([
        { label: "Ver cómo se escribe", cls: "ghost", onClick: function () {
            settled = true;
            writers.forEach(function (x, i) {
              x.cell.classList.add("revealed");
              setTimeout(function () { x.w.animateCharacter(); }, i * 700);
            });
            ctx.setActions([]);
            ctx.feedback(false, buildAnswerNote(c));
            ctx.autoGrade(false);
          } },
      ]);
    }
